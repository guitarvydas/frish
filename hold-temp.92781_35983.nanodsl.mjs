...
pretransmogrifier {
  ...
  ScopedSubr = "defsubr" subrname str "{" #subrinnard* "}"
  subrinnard =
    | "{" subrinnard* "}"     -- rec
...
enter_rule ("ScopedSubr");
    set_return (`\ndefsubr ${name.rwr ()} ${str.rwr ()} { ${innards.rwr ().join ('')} } `);
return exit_rule ("ScopedSubr");
},
subrinnard_rec : function (lb,innards,rb,) {
enter_rule ("subrinnard_rec");
    set_return (`{${innards.rwr ().join ('')}}`);
return exit_rule ("subrinnard_rec");
},
subrinnard_pattern : function (p,) {
enter_rule ("subrinnard_pattern");
    set_return (`${p.rwr ()}`);
return exit_rule ("subrinnard_pattern");
},
subrinnard_other : function (c,) {
enter_rule ("subrinnard_other");
    set_return (`${c.rwr ()}`);
return exit_rule ("subrinnard_other");
},
Pattern_memomacro : function (_defsyn,name,_def,e,) {
enter_rule ("Pattern_memomacro");
    set_return (` ${memomacro (`${name.rwr ()}`,`${e.rwr ()}`,)}${xcontinue ()}`);
